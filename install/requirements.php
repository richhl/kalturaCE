...
{
  global $requirements_stats,$reqs_error;
  exec('php -v',$output, $error);
  if ($error)
  {
    exec('/opt/lampp/bin/php -v',$output,$error);
    if ($error) {
      $reqs_error = false;
      return '<li class="verify failed">You must have php-cli. &nbsp;This is required in order to run batch jobs, etc.<br />Please install php-cli and make sure that php executable is in PATH</li>';
    }
  }
  $requirements_stats++;
...
function verify_memcache()
{
  ...
  if (!$connection)
  {
    $output = '<li class="verify optional">memcache is recommended for better performance, but it is optional. &nbsp;You can continue installation without it.<br />'.
...
function verify_ffmpeg()
{
  ...
  $exec_ffmpeg = '../kaltura/bin/i386/ffmpeg -version';
  exec($exec_ffmpeg, $output, $error);
  if ($error)
  {
    $reqs_error = false;
    return '<li class="verify failed">Could not run FFMPEG. &nbsp;FFMPEG is crucial for converting media to FlashVideo format. &nbsp;The system should be able to run the script located at: <br />'.$root_dir.'kaltura/bin/i386/ffmpeg - please make sure apache can run that script.</li>';
  }
