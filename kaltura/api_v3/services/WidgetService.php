...
class widgetService extends KalturaBaseService 
{
	...
	function addAction( KalturaWidget $widget )
	{
		if ( $widget->kshowId )
		{
			$kshow = kshowPeer::retrieveByPK( $widget->kshowId );
			if ( ! $kshow )
				throw new KalturaAPIException ( APIErrors::KSHOW_DOES_NOT_EXISTS );
		}
		if ( $widget->entryId )
		{
			$entry = entryPeer::retrieveByPK( $widget->entryId );
			if ( ! $entry )
				throw new KalturaAPIException ( APIErrors::INVALID_ENTRY_ID , "entry" , $widget->entryId );
		}
