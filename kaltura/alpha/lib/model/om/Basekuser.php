...
abstract class Basekuser extends BaseObject  implements Persistent {
		...
		if ($this->collBBPosts === null) {
			if ($this->isNew()) {
			   $this->collBBPosts = array();
				...
				$criteria->add(BBPostPeer::KUSER_ID, $this->getId());
	...
	public function getBBPostsJoinBBForum($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseBBPostPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collBBPosts === null) {
			if ($this->isNew()) {
				...
				$this->collBBPosts = BBPostPeer::doSelectJoinBBForum($criteria, $con);
			}
		} else {
