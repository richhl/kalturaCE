...
abstract class BaseMailJob extends BaseObject  implements Persistent {
	...
	public function setProcessorName($v)
	{
		...
		if ($this->processor_name !== $v) {
			...
			$this->modifiedColumns[] = MailJobPeer::PROCESSOR_NAME;
		}
	...
	public function setProcessorLocation($v)
	{
		...
		if ($this->processor_location !== $v) {
			...
			$this->modifiedColumns[] = MailJobPeer::PROCESSOR_LOCATION;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->processor_location = $rs->getString($startcol + 17);

			$this->processor_expiration = $rs->getTimestamp($startcol + 18, null);

			$this->execution_attempts = $rs->getInt($startcol + 19);

			$this->lock_version = $rs->getInt($startcol + 20);

			$this->partner_id = $rs->getInt($startcol + 21);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[17] => $this->getProcessorLocation(),
			$keys[18] => $this->getProcessorExpiration(),
			$keys[19] => $this->getExecutionAttempts(),
			$keys[20] => $this->getLockVersion(),
			$keys[21] => $this->getPartnerId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[17], $arr)) $this->setProcessorLocation($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setProcessorExpiration($arr[$keys[18]]);
		if (array_key_exists($keys[19], $arr)) $this->setExecutionAttempts($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setLockVersion($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setPartnerId($arr[$keys[21]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MailJobPeer::PROCESSOR_NAME)) $criteria->add(MailJobPeer::PROCESSOR_NAME, $this->processor_name);
		if ($this->isColumnModified(MailJobPeer::PROCESSOR_LOCATION)) $criteria->add(MailJobPeer::PROCESSOR_LOCATION, $this->processor_location);
