...
class alert extends Basealert
{
	...
	public function sendEmailAlert()
	{
		...
		{
			$mailjob = new MailJob();
		 	$mailjob->Initialize( $this->getAlertType() );
		 	$mailjob->setFromEmail( kconf::get ( "batch_notification_sender_email" ) );
		 	$mailjob->setFromName(kconf::get ( "batch_notification_sender_name" ));
		 	$mailjob->setSubjectParamsArray( $this->getSubjectParamsArray() );
			$mailjob->setBodyParamsArray( $this->getBodyParamsArray() );
			$mailjob->setRecipientEmail( $this->kuser->getEmail() );
			$mailjob->save();
		}
