...
class myBatchBulkUpload extends myBatchBase
{
	...
	public function handleVerifyCSV() 
	{
		...
		foreach($jobs as $job)
		{
			...
			$fileLocation = $jobData["fileLocation"];
	...
	public function handleParseCSV()
	{
		...
		foreach($jobs as $job)
		{
			...
			// init the log file
			$logFile = "/content/batchfiles/".$partnerId."/"."log_".$jobId."_".date("Y-m-d_H-i-s", time()).".csv";
			kFile::setFileContent(myContentStorage::getFSContentRootPath().$logFile, "");
			
			// open the log file
			$logFileHandle = fopen(myContentStorage::getFSContentRootPath().$logFile, "w");
	...
	public function handleAddEntries() 
	{
		...
		foreach($jobs as $job)
		{
			...
			$logFileLocation = $jobData["logFile"];
			TRACE('Reading file: "' . myContentStorage::getFSContentRootPath().$logFileLocation . '"');
			$logFileHandle = fopen(myContentStorage::getFSContentRootPath().$logFileLocation, "r+");
...
require_once(SF_ROOT_DIR.DIRECTORY_SEPARATOR.'apps'.DIRECTORY_SEPARATOR.SF_APP.DIRECTORY_SEPARATOR.'modules/partnerservices2/actions/addentryAction.class.php');
					...
					{
						$wereErrors = true;
						$err = "Failed to create entry (".$res["error"][0].")";
				...
				$logFileHandle = fopen(myContentStorage::getFSContentRootPath().$logFileLocation, "w");
