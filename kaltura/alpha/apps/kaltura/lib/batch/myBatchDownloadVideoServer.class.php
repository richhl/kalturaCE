...
//		$data_path = myContentStorage::getFSContentRootPath() . "/" . str_replace( "/data/" , "/download/" , $entry->getDataPath() );
...
TRACE ( "Before sending notification [" . notification::NOTIFICATION_TYPE_BATCH_JOB_SUCCEEDED . "]" );				
				$not_res = myNotificationMgr::createNotification( notification::NOTIFICATION_TYPE_BATCH_JOB_SUCCEEDED, $job , null , null , null , 
					array ( "download_url" => $downloadUrl ) );				
TRACE ( "After sending notification [" . notification::NOTIFICATION_TYPE_BATCH_JOB_SUCCEEDED . "] notification: " . 
	...
	protected static function sendDownloadMail ( $email , $admin_name , $entry , $format )
	{
		...
		$download_url = requestUtils::getCdnHost().$finalPath ;
TRACE ( "Sending email to [$admin_name]:[$email] about entry [{$entry->getId()}] with format [$format]->[$download_url]");
		...
		// TODO - create mail templates and set the '$finalPath'
	  	$mailjob = new MailJob();
	 	$mailjob->Initialize( self::KALTURAS_DOWNLOAD_READY );
	 	$mailjob->setFromEmail( kConf::get ( "batch_download_video_sender_email" ) ) ;// download_video@kaltura.com');
	 	$mailjob->setFromName( kConf::get ( "batch_download_video_sender_name" ) ) ; //'Kaltura');
	 	$mailjob->setBodyParamsArray( array( $admin_name , $download_url ) );
		$mailjob->setSubjectParamsArray( array() );
		$mailjob->setRecipientEmail( $email );
		$mailjob->save();		
	}
}
