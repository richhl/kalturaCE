...
require_once ( "infra/myFlvHandler.class.php");
...
class myFileConverter
{
	...
	private static $AUDIO_EXT = array ( "wav" , "mp3" , "wma" , "au" , "ra" , "aac" );
	...
	static public function getRed5Duration ( $source_file )
	{
		...
		try
		{
			...
			$xml_doc->loadXML( kFile::getFileContent( $source_file . '.meta') );
...
//TRACE ( "Before calling ffmpegConvert:\n[$fixed_source_file]->[$target_file]")		;
		...
		$konverted = false;
		/*
		 if ( $return_value != 0 && kString::endsWith ( $source_file , ".flv" ) )
		 {
			// try to Konvert this file to be a good enough FLV file
			// TODO - add metadata to the target_file before copying it
			$metadata = array( "duration" => $conversion_info->duration );

			self::setDurationForFailedFlv ( $conversion_info , $source_file , $text_output_file );

			//myContentStorage::moveFile( $source_file , $target_file , true , true );
			//write to the log file that this is not a regular convertion!
			$log_content = kFile::getFileContent( $text_output_file );
			$log_content .= "\n\nKonverted file !!!";
			kFile::setFileContent( $text_output_file, $log_content );

			// act  as if the conversion worked ok
			$return_value = 0;
			$konverted = true;
			}
			*/
...
//		$conversion_string .= " -lavfopts i_certify_that_my_video_stream_does_not_use_b_frames " ;
	...
	{
		$conversion_string_file_name = $file_name . ".conversionString";
		$result = @file_put_contents( $conversion_string_file_name , $conversion_string );
		return $result;
	}
	...
	static public function createImageByFile($source_file)
	{
		...
		{
			kLog::log( "changeImageSize:: file not found [$source_file]" ) ;
			return null;	
		}
	...
	static public function changeImageSize( $source_file , $target_file , $target_type = "image2" , $width = 0, $height = 0 , $crop=false )
	{
		...
		{
			sfLogger::getInstance()->log ( "changeImageSize:: Cannot change image size: $source_file [$sourcewidth x $sourceheight]" , 'warning' );
		}
	}
	...
	static public function createImageThumbnail ( $source_file , $target_file , $target_type = "image2" , $width = self::DEFAULT_THUMBNAIL_WIDTH , $height = self::DEFAULT_THUMBNAIL_HEIGHT )
	{
		if ( ! file_exists( $source_file ) )
		{
			sfLogger::getInstance()->err ( __CLASS__ . " File not found [$source_file]" );
			return;
		}

		if ( is_dir( $source_file ))
		{
			sfLogger::getInstance()->err ( __CLASS__ . " Cannot create image from directory [$source_file]" );
			return;
		}
		//$text_output_file = self::createLogFileName ($source_file );
		...
		{
			kLog::log ( __CLASS__ . " bad image / dimensions [$source_file]" );
			return;
		}
	...
	$src_x = 0, $src_y = 0, $src_w = 0, $src_h = 0)
	{
		...
		if (!$srcIm)
		{
			kLog::log("convertImage - failed to load image [$source_file] while creating [$target_file]");
			return null;
		}
		...
		{
			imagedestroy($srcIm);
			return self::convertImageRectangleCrop($source_file, $target_file, $width, $height, $bgcolor, $quality, $src_x, $src_y, $sourcewidth, $sourceheight);
		}
		...
		{
			kLog::log("convertImage - calculated dimensions [$new_width]x[$new_height] while creating [$target_file]");
			return null;
		}
		...
		if (!$im)
		{
			kLog::log("convertImage - failed to create image [$new_width]x[$new_height] while creating [$target_file]");
			return null;
		}
	...
	private static function convertImageRectangleCrop($source_file, $target_file, $width, $height, $bgcolor, $quality,
	$src_x = 0, $src_y = 0, $src_w = 0, $src_h = 0)
	{
		list($sourcewidth, $sourceheight, $type, $attr, $srcIm) = self::createImageByFile($source_file);
		...
		$im = imagecreatetruecolor( $width, $height );
		imagecopyresampled( $im, $srcIm, 0, 0, $src_x + $width_crop / 2, $src_y + $height_crop / 2, $width, $height, $sourcewidth - $width_crop, $sourceheight - $height_crop );
		imagedestroy($srcIm);
		self::saveImageByType($im, $type, $target_file, $quality);
		imagedestroy($im);
		return $target_file;
	...
	$quality = 0, $src_x = 0, $src_y = 0, $src_w = 0, $src_h = 0)
	{
		...
		$temp_array = split("_", $crop_provider);
		$class_name = "";
