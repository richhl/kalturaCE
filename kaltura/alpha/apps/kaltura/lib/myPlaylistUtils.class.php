 ...
 */	
	...
	{
		if ( ! $playlist )	 throw new APIException ( "No playlist to validate" );
...
//			$entry_id = "";
		...
		elseif ( $playlist->getMediaType() == entry::ENTRY_MEDIA_TYPE_XML )
		{
			...
			kLog::log( "Playlist [" . $playlist->getId() . "] [" . $playlist->getName() . "] dataContent:\n" . $dynamic_playlist_str );
		...
		{
			throw new APIException ( APIErrors::INVALID_PLAYLIST_TYPE  );
		}
	 ...
	 * 
	 */
	public static function executePlaylistById ( $partner_id , $playlist_id ,  $extra_filters = null , $detailed = false )
	{
		...
		{
			throw new APIException( APIErrors::INVALID_ENTRY_TYPE , entry::ENTRY_TYPE_PLAYLIST ) ;
		}
		
		return self::executePlaylist ( $partner_id , $playlist ,  $extra_filters , $detailed );
	}
	
	public static function executePlaylist ( $partner_id , $playlist ,  $extra_filters = null , $detailed = false )
	{
		if ( ! $playlist )
		{
			throw new APIException( APIErrors::INVALID_ENTRY_ID , "Playlist" , "" ) ; 
		}
	...
	public static function executeStaticPlaylist ( entry $playlist , $extra_filters , $detailed = false )
	{
/*		
		$c = new Criteria();
		$c->add ( entryPeer::STATUS , entry::ENTRY_STATUS_READY ); 
		$entry_list = roughcutEntryPeer::retrievEntriesByRoughcutIds( $c , $playlist_id );
	*/
		// for now - read the entry list from the file.
		...
		if ( $entry_id_list )
		{
			...
			foreach ( $entry_id_list as &$entry_id ) $entry_id=trim($entry_id);
		}
		else
		{
			return null;//array();
		...
		//$unsorted_entry_list = entryPeer::retrieveByPKs( $entry_id_list );
		...
		if ( $extra_filters && isset ( $extra_filters[1] )  )
		{
			...
			// read the _eq_dispaly_in_search field but ignore it because it's part of a more complex criterion - see bellow
			$display_in_search = $entry_filter->get( "_eq_dispaly_in_search");
			if ( $display_in_search >= 2 )
			{
				$entry_filter->set ( "_eq_dispaly_in_search" , null );
			}
			
//			$entry_filter->forceMatch(); // use the MATCH mechanism
 ...
 */	
	public static function executeDynamicPlaylist ( $partner_id , $xml , $extra_filters = null ,$detailed = true )
	{
		...
		foreach ( $entry_filters as $entry_filter )
		{
			...
			// read the _eq_dispaly_in_search field but ignore it because it's part of a more complex criterion
			$display_in_search = $entry_filter->get( "_eq_dispaly_in_search");
			if ( $display_in_search >= 2 )
			{
				$entry_filter->set ( "_eq_dispaly_in_search" , null );
...
//			$entry_filter->forceMatch(); // use the MATCH mechanism
	...
	private static function getPlaylistFilterListStruct ( $xml )
	{
		try
		{
