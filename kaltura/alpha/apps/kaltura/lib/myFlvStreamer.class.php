...
class myFlvStreamer
{
	...
	private static function getAssets($filePath, $timeline, $streamNum, $addPadding)
	{
		...
		{
			kLog::log ( "Cannot find XML file at [" . $contentRoot.$filePath . "],  timeline:$timeline, streamNum:$streamNum"  );
			return null;
		}
		...
		foreach ( $assets as $asset )
		{
			...
			foreach ( $stream_info_list as $stream_info)
			{
				...
				{
					kLog::log ("qqq INVALID ENTRY $file_name");
				...
				{
					$assetArray[] = array("file_name" => $contentRoot.$file_name,
	...
	private  static function iterateAssets ( $assets , $streamInfo , $addPadding, $echo )
	{
		...
		foreach ( $assets as $asset )
		{
			...
			$use_multi_flavor = myFlvStaticHandler::isMultiFlavor ( $asset['file_name'] ) && $timeline == "video";
			...
			for ( $i=0 ; $i < $number_of_iterations ; $i++ )
			{
				if ( $use_multi_flavor )
				{
					if ( $i == 0 )
					{
						$file_name = myContentStorage::getFileNameEdit( $asset['file_name'] );  // edit flavor with original attributes
						$start_byte = $asset['start_byte'];
						$end_byte = $asset['end_byte'];
					}
					else
					{
						$file_name = $asset['file_name'];	// original file name with '_play' attributes
						$start_byte = $asset['start_byte_play'];
						$end_byte = $asset['end_byte_play'];
					}

					kLog::log( "myFlvStreamer:: ($i) using $file_name ($start_byte - $end_byte)" );

				}
				else
				{
					$file_name = $asset['file_name'];		// original file name with original attributes
					$start_byte = $asset['start_byte'];
					$end_byte = $asset['end_byte'];
				}
...
self::log ( __METHOD__ , "in loop" , $asset_id );
			...
			if ( $ext == NULL  )
  			...
  			foreach ( $stream_info_list as $stream_info)
  			{
  				...
  				try
  				{
  					$stream_info->setAttribute ( "file_name",  $entry->getDataPath() );
		  			$start_byte = $stream_info->getAttribute ( "start_byte" );
		  			$end_byte = $stream_info->getAttribute ( "end_byte" );
							...
							if ( $use_multi_flavor )
							{
								...
								if( $result )
								{
									if ( $start_time_play != $start_time )
									{
										...
										// 	edit - more keyfrmaes !
										$flv_file_name_edit = myContentStorage::getFileNameEdit( $flv_file_name );
...
*/
	...
	{
		$file_name = $filePath . ".". $streamInfo[1] . $streamInfo[2] . ".metadata" . ($addPadding ? "-padding" : "") ;
		file_put_contents( $file_name , $metadata_content );
	}
