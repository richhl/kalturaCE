...
//				$criterion->addAnd ( $criterion2 ) ;
	...
	public static function getCurrentConversionProfile ( $partner_id  )
	{
		...
		if( !$conversion_profile_id)
		{
			// will return the partner default profile and if not found - the system default profile
...
/*@Partner $partner*/
	...
	*/
	...
	{
		$three_days_grace = time() - (dateUtils::DAY * 3);
		...
		list ( $totalStorage , $totalUsage , $totalTraffic ) = myPartnerUtils::collectPartnerUsage($partner, $partnerPackage, $report_date);
			...
			!$partner->getEightyPercentWarning())
		{
			...
			$body_params = array ( $partner->getAdminName(), $totalUsageGB , '$50' );
		...
		elseif ($percent >= 120 &&
				...
				$partner->getUsageLimitWarning() <= $three_days_grace)
		{
			$body_params = array ( $partner->getAdminName(), $totalUsageGB );
	...
	public static function getPartnerUsageGraph( $year , $month , $partner , $resolution = 'days')
	{
		...
		else
		{
			$c->addAnd ( PartnerActivityPeer::ACTIVITY , PartnerActivity::PARTNER_ACTIVITY_MONTHLY_AGGREGATION );
			$activity = PartnerActivityPeer::doSelect( $c );
			$graph_points['line'] = myPartnerUtils::year_activity_to_graph($activity);
		}
		$strGraphLine = '';
	...
	public static function year_activity_to_graph($act)
	{
		$points = array();
		foreach($act as $activity)
		{
			$date = explode('-', $activity->getActivityDate());
			$points[(int)$date[1]] = round($activity->getAmount2()/1024); // Amount2 is in KB, converting to MB
		}
