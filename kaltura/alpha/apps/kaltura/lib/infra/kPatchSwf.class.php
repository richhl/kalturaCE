...
class kPatchSwf
{
	...
	const KALTURA_EMBED_SIGNATURE = "\0\0\0\0KALTURA_EMBEDDED_DATA";
	...
	public function kPatchSwf($filename)
	{
	...
	public function patch($data, $dest_filename = null)
	{
		...
		while(list($tag_start_pos, $tag_type, $tag_header_len, $tag_len) = $this->getNextTag())
		{
			...
			{
				$signature = substr($swfdata, $tag_start_pos + $tag_header_len + 2 + 4, strlen(self::KALTURA_EMBED_SIGNATURE));
				if ($signature != self::KALTURA_EMBED_SIGNATURE)
					continue;
				...
				if ($dest_filename)
				{
					file_put_contents($dest_filename, $header.$zdata);
				}
				else
				{
					echo $header;
