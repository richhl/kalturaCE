...
// ----------------------------------------------------------------
	...
	{
		$url = "http://www.jamendo.com/get/track/search/track-album-artist/audio/ahahlist/".urlencode($searchText);
...
//		$response = $this->hitUrl ( $url , $postData );
		...
		foreach($response as $sound)
		{
			$sounds[] = array(
				'url' => $sound->t,
				'title' => $sound->trn.' ('.$sound->aln.') by '.$sound->ard,
				'id' => $sound->id);
		}
