	...
	public  static function setDisplayInSearch ( BaseObject $obj , $parent_obj = null )
	{
		...
		if ( $obj->isNew() || $obj->getDisplayInSearch() === null )
		{
			...
			if ( $res )
			{
				...
				{
					// allow entries that the permissions and NOT private
					self::setRes ( $res , $obj->getPermissions () != myPrivilegesMgr::PERMISSIONS_PRIVATE );
					
					// status=READY , type=MEDIACLIP, view permissions of kshow 
					self::setRes ( $res , $obj->getStatus() == entry::ENTRY_STATUS_READY );
					// TODO -  allow clips or roughcuts
					self::setRes ( $res , $obj->getType() == entry::ENTRY_TYPE_MEDIACLIP );
					// self::setRes ( $res , ( $obj->getType() == entry::ENTRY_TYPE_MEDIACLIP || $obj->getType() == $obj->getType() == entry::ENTRY_TYPE_SHOW ) );
					if ( $res )
					{		
						// TODO - remove KSHOW					
						$kshow = $parent_obj;
						...
						if ( $kshow )
						{
							self::setRes ( $res ,  $kshow->getViewPermissions() == kshow::KSHOW_PERMISSION_EVERYONE || $kshow->getViewPermissions() == null );
						}
					}
				}
				else
...
//	echo __METHOD__ . " (" . get_class ( $obj ) . ") res [$res]\n";
	...
	{ 
		// a  single quiote should be removed with no space replacer
	...
	{
		$prefix = $res > 1 ? "_KAL_NET_" : "_PAR_ONLY_" ; 
