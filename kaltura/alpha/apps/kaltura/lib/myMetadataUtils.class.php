...
class myMetadataUtils
{
	...
	static public function getExtData($kshow_id, $partner_id)
	{
		...
		$file_name = myContentStorage::getFSContentRootPath(). "/" . $show_entry->getDataPath();

		if ($show_entry->getMediaType() != entry::ENTRY_MEDIA_TYPE_SHOW)
			return array(null, null);

		$content = kFile::getFileContent( $file_name );
	...
	static public function setMetadata($content, $kshow, $show_entry , $ignore_current = false, $version_info = null )
	{
		...
		$current_content = ($show_entry->getMediaType() != entry::ENTRY_MEDIA_TYPE_SHOW) ? "" : kFile::getFileContent( $file_name );
		...
		if ( $content != $current_content )
		{
			...
			$total_duration = 0;
			$editor_type = null;
			$content = 	myFlvStreamer::fixMetadata ($content, $version , $total_duration , $editor_type);
			...
			kFile::setFileContent ( $file_name , $content );
			...
			$xml_content = kFile::getFileContent( $file_name );
			$update_kshow = true;

			$show_entry_id = $show_entry->getId();
//sfLogger::getInstance()->log ( __METHOD__ . ":[$show_entry_id]" );
			// update the roughcut_entry table
			$all_entries_for_roughcut = self::getAllEntries ( $content );
//sfLogger::getInstance()->log ( __METHOD__ . ":[$show_entry_id]:" . implode ( "," , $all_entries_for_roughcut ) );
...
//				$entry->setUpdateWhenReady ( $current_kshow_version );
		...
		{
			kLog::log ( "Exceeded number of entries in metadata [" . self::MAX_ENTRIES_IN_METADATA . "]. Will not add entry [{$entry_id}] to file." );
			$should_save = false;
			return null;
		}
...
//		echo $temp_xml_doc->saveXML( );
		...
		{
			kLog::log ( "No VideoAssets parent node for entry [{$entry_id}] content [{$content}]." );
			return null;
		}
	...
	public static function updateAllMetadataVersionsRelevantForEntry ( $entry , $version_to_update )
	{
		...
		$logger->log ( "updateAllMetadataVersionsRelevantForEntry [" . $entry->getId() . "] version_to_update [$version_to_update] status [$status]");
		...
		$logger->log ( "updateAllMetadataVersionsRelevantForEntry [" . $entry->getId() . "] updated " . implode ("," , $versions_updated ) );
