...
// TODO - add width & height after fixinf entry->getWidth mechanism 
	public function renderEntry ( $entry )
	{
		...
		$kaltura_elements =
			...
			"<kaltura:views>" . $entry->getViews() . "</kaltura:views>" .  
			"<kaltura:plays>" . $entry->getPlays() . "</kaltura:plays>" .
			...
			"<kaltura:votes>" . $entry->getVotes() . "</kaltura:votes>" .
			"<kaltura:rank>" . $entry->getRank() . "</kaltura:rank>" .	
		 ...
		 $mrss = '<item>' . 
		 	...
		 	'<media:content ' . 
               ...
               'fileSize="' . $entry->getFileSize() . '" ' .  
		 		( $entry->getType() == entry::ENTRY_MEDIA_TYPE_VIDEO ? 'type="video/x-flv" ' : '  ' ) . 
...
//              	'width="' . $entry->getWidth() . '" ' .  
               	...
               	'<media:thumbnail url="'. $entry->getThumbnailUrl() . '"/>' . 
