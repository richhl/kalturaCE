...
//			kConversionHelper::fillConversionParams ( $source_file , $conv_params );
	...
	protected function archiveFile ( $file_name  )
	{
		...
		TRACE ( "Archiving file [" . $file_name . "] to dir [" . $archive_dir . "]}"  );
		$target = self::createFilePath ( $archive_dir , $file_name );
		...
		if ( ! file_exists ( $file_name ) && file_exists( $target ) ) return $target;
