...
sfLogger::getInstance()->log ( __METHOD__ . " kshow_id [$kshow_id] entry_id [$entry_id]" );
...
$this->benchmarkStart( "list_type_kuser" );
		if ( $list_type & self::LIST_TYPE_KUSER )
		{
			...
			if ( $puser_kuser && $puser_kuser->getKuserId() )
			{
				...
				$c->addAnd ( entryPeer::TYPE , entry::ENTRY_TYPE_MEDIACLIP );
//				$c->addAnd ( entryPeer::MEDIA_TYPE , entry::ENTRY_MEDIA_TYPE_SHOW , Criteria::NOT_EQUAL );
				$c->addAnd ( entryPeer::KUSER_ID , $puser_kuser->getKuserId()  );
				...
				$strEntries = @unserialize($kuser->getPartnerData());
				if ($strEntries)
				{
...
$this->benchmarkStart( "list_type_roughcut" );
		...
		if ( $list_type & self::LIST_TYPE_ROUGHCUT )
		{
			...
			{
				$roughcut_file_name =  $entry->getDataPath( $version );
