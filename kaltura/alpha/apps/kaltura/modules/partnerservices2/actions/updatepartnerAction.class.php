		...
		if ( count ( $fields_modified ) > 0 )
		{
			...
			if ( $partner && $target_partner )
			{
				if ( @$fields_modified["adminEmail"] && $target_partner->getAdminEmail() != $fields_modified["adminEmail"]) {
					myPartnerUtils::emailChangedEmail($target_partner->getAdminEmail(), $fields_modified["adminEmail"], $target_partner->getName(), updatepartnerAction::KALTURAS_PARTNER_EMAIL_CHANGE);
				}
