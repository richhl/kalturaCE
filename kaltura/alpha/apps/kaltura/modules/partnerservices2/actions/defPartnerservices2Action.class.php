...
//				$this->addError ( $api_ex->api_code ,$api_ex->extra_data );
	...
	private function setServiceConfigFromPartner (  $partner )
	{
		...
		if ( $partner && $partner->getStatus() == Partner::CONTENT_BLOCK_STATUS )
		{
	...
	private function getPuserKuser ( $partner_id , $subp_id, $puser_id )
	{
		...
		$puser_kuser = $this->createUserOnDemand ($partner_id , $subp_id, $puser_id , $puser_kuser );
...
//				$this->addError ( "puser ($partner_id,$puser_id) does not have access privileges to kshow [$kshow_id]" );
	...
	protected function createUserOnDemand ( $partner_id , $subp_id, $puser_id  , $puser_kuser )
	{
		...
		if ( $create_user )
		{
			...
			$puser_kuser = PuserKuserPeer::createPuserKuser ( $partner_id , $subp_id, $puser_id , $kuser_name , $puser_name, false  );
		}
