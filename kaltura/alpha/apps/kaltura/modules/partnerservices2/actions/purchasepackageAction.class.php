	...
	public function executeImpl ( $partner_id , $subp_id , $puser_id , $partner_prefix , $puser_kuser )
	{
		...
		if("Success" == $httpParsedResponseAr["ACK"]) {
			...
			if ($partner->getStatus() == 2) {
		  		$mailjob->Initialize( PartnerPackages::KALTURA_PACKAGE_UPGRADE );
			 	$mailjob->setBodyParamsArray( array( $partner->getAdminName(), $package['name'], $package['cycle_bw'] , $package['cycle_fee'] ) );
			}
			else
			{
		  		$mailjob->Initialize( PartnerPackages::KALTURA_LOCKED_PACKAGE_UPGRADE );
  			 	$mailjob->setBodyParamsArray( array( $partner->getAdminName(), $package['name'], $package['cycle_bw'] , $package['cycle_fee'] ) );
			}
		 	$mailjob->setFromEmail( kConf::get( "purchase_package_email" ));
		 	$mailjob->setFromName( kConf::get( "purchase_package_name" ) );
