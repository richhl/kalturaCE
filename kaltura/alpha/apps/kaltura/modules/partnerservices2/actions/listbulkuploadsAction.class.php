	...
	public function executeImpl ( $partner_id , $subp_id , $puser_id , $partner_prefix , $puser_kuser )
	{
		...
		foreach($jobs as $job) 
		{
			...
			$obj[] = array (
				...
				"uploadedOn" => $jobData["uploadedOn"],
				"numOfEntries" => @$jobData["numOfEntries"],
				"status" => $job->getStatus(),
				"error" => @$jobData["error"],
				"logFileUrl" => (@$jobData["logFile"]) ? requestUtils::getCdnHost() . $jobData["logFile"] : "",
				"csvFileUrl" => (@$jobData["fileRelativeLocation"]) ? requestUtils::getCdnHost() . $jobData["fileRelativeLocation"] : ""
			);
