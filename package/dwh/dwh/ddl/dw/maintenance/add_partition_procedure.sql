...
DROP PROCEDURE IF EXISTS `kalturadw`.`add_events_partition_for_fact_event`$$
CREATE DEFINER=`etl`@`localhost` PROCEDURE `kalturadw`.`add_events_partition_for_fact_event`()
BEGIN
	...
	WHILE (p_continue) DO
		...
		IF (_current_date > p_date - INTERVAL 1 MONTH AND p_name is not null) THEN
			SET @s = CONCAT('alter table kalturadw.dwh_fact_events ADD PARTITION (partition p_' ,p_name ,' values less than (', p_value ,'))');
			PREPARE stmt FROM  @s;
			EXECUTE stmt;
...
BEGIN
	...
	# a special case - dwh_fact_events has a different key for the partition - use the dedicated function
	CALL kalturadw.add_events_partition_for_fact_event();

	# for the rest of the partitioned tables
...
#	CALL kalturadw.add_partition_for_table('dwh_aggr_monthly_partner');
