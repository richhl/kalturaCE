...
abstract class KJobConversionEngine extends KConversionEngine
{
	...
	protected function getExecutionCommandAndConversionString ( KalturaConvertJobData $data )
	{
		...
		foreach ( $cmd_line_arr as $type => $cmd_line )
		{
			...
			foreach($cmdArr as $index => $cmd)
			{
				...
				if($lastIndex > $index)
				{
					$uniqid = uniqid("tmp_convert_{$this->entryId}_");
	...
	public function simulate ( KalturaSourceJobData $data )
	{
	...
	public function convert ( KalturaSourceJobData &$data )
	{
	...
	public function convertJob ( KalturaConvertJobData &$data )
	{
		...
		self::addToLogFile ( $log_file , "Executed by [" . $this->getName() . "] flavor params id [" . $data->flavorParamsOutput->flavorParamsId . "]" ) ;
		...
		self::logMediaInfo ( $log_file , $data->destFileSyncLocalPath );
