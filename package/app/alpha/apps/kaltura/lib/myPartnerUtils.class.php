...
//				$criterion->addAnd ( $criterion2 ) ;
	...
	public static function getConversionProfile2ForEntry ( $entry_id  )
	{
		...
		kLog::log("getConversionProfile2ForEntry: conversion_profile_2_id [$conversion_profile_2_id]");
		if ( ! $conversion_profile_2_id )
		{
			...
			kLog::log("getConversionProfile2ForEntry: conversion_quality [$conversion_quality]");
...
/*@Partner $partner*/
	...
	public static function daily_activity_to_graph($act, $res, $start_date)
	{
		...
		foreach ($act as $row)
		{
			...
			if (!isset($points[(int)$date[2]]))
			{
				$points[(int)$date[2]] = 0;
			}
			$points[(int)$date[2]] += round(($row->getAmount()/1024)); // normalize to MB
		}
 	...
 	{
 		kLog::log("copyEntriesByType - Copying entries from partner [".$fromPartner->getId()."] to partner [".$toPartner->getId()."] with type [".$entryType."]");
 	...
 	{
 		kLog::log("copyUiConfsByType - Copying uiconfs from partner [".$fromPartner->getId()."] to partner [".$toPartner->getId()."] with type [".$uiConfType."]");
 		entryPeer::setUseCriteriaFilter ( false );
 		$c = new Criteria();
 		...
 		foreach($uiConfs as $uiConf)
 		{
 			...
 			$newUiConf->save();
 			kLog::log("copyUiConfsByType - UIConf [".$newUiConf->getId()."] was created");
 		}
 	}
 	...
 	public static function copyConversionProfiles(Partner $fromPartner, Partner $toPartner)
 	{
		$copiedList = array();
 		...
 		kLog::log("copyConversionProfiles - Copying conversion profiles from partner [".$fromPartner->getId()."] to partner [".$toPartner->getId()."]");
 		...
 		foreach($conversionProfiles as $conversionProfile)
 		{
 			...
 			kLog::log("copyConversionProfiles - Copied [".$conversionProfile->getId()."], new id is [".$newConversionProfile->getId()."]");
 	...
 	{
		$copiedList = array();
 		kLog::log("copyAccessControls - Copying access controls from partner [".$fromPartner->getId()."] to partner [".$toPartner->getId()."]");
 		...
 		foreach($accessControls as $accessControl)
 		{
 			...
 			kLog::log("copyAccessControls - Copied [".$accessControl->getId()."], new id is [".$newAccessControl->getId()."]");
