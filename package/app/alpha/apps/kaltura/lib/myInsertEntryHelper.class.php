...
class myInsertEntryHelper
{
	...
	{
		kLog::log("handleEntry($type, $entry_id, $name)");
		...
		kLog::log("handleEntry: media_source: $media_source, prefix: $prefix");
		...
		else
		{
			...
			kLog::log("handleEntry: media_type: $media_type");
...
*/
				{
					...
					{
						kLog::log("Failed downloading file[$entry_url]");
		...
		if ($entry_status == entry::ENTRY_STATUS_PRECONVERT && !myContentStorage::fileExtNeedConversion($ext))
		{
			...
			kLog::log("handleEntry: err: $this->errorMsg");
		...
		if ($entry_status == entry::ENTRY_STATUS_ERROR_IMPORTING)
		{
			...
			kLog::log("handleEntry: error importing, thumb not needed");
		}
		else
		{
			...
			kLog::log("handleEntry: handling media $media_type");
			if ($media_type == entry::ENTRY_MEDIA_TYPE_IMAGE)
			{
...
//		$entry->setDescription('');//$this->getParam('entry_description'));
		...
		{
			kLog::log("handleEntry: saving none audio thumb [$thumbBigFullPath]");
		...
		if ($entry->getStatus() == entry::ENTRY_STATUS_IMPORT)
 		{
 			kLog::log("handleEntry: creating import job");
...
// 			$batchClient->addJob($this->entry_id, $entry_url, $entry_fullPath);
		...
		else if ($entry->getStatus() == entry::ENTRY_STATUS_PRECONVERT )
		{
			if ( ! $skip_conversion )
			{
 				...
 				kLog::log("handleEntry: creating original flavor asset for pre convert");
				...
				if($flavorAsset)
				{
					...
					kJobsManager::addConvertProfileJob(null, $this->entry_id, $this->partner_id, $flavorAsset->getId(), $inputFileSyncLocalPath);
				}
				else
				{
					$entry->setStatus(entry::ENTRY_STATUS_ERROR_CONVERTING);
...
//				}
		...
		else if ($entry->getStatus() == entry::ENTRY_STATUS_READY || $entry->getStatus() == entry::ENTRY_STATUS_MODERATE )
		{
			...
			{
	 			kLog::log("handleEntry: creating original flavor asset for ready entry");
				$flavorAsset = kFlowHelper::createOriginalFlavorAsset($this->partner_id, $this->entry_id);
				if($flavorAsset)
				{
...
//					kBusinessPreConvertDL::bypassConversion($flavorAsset, $entry);
			...
			{
				kLog::log("handleEntry: creating data file sync for file [$entry_fullPath]");
