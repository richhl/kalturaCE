...
class kBusinessConvertDL
{
	public static function parseFlavorDescription(flavorParamsOutputWrap $flavor)
	{
		...
		if(is_array($flavor->_errors) && count($flavor->_errors))
		{
			...
			kLog::log("Flavor errors: $errDesc");
			$description .= $errDesc;
		}
		...
		if(is_array($flavor->_warnings) && count($flavor->_warnings))
		{
			...
			kLog::log("Flavor warnings: $errDesc");
			$description .= $errDesc;
		}
		return $description;
	}
	...
	{
		kLog::log("Filter Tag Flavors, " . count($flavors) . " flavors supplied");
		...
		if($hasComplied)
			kLog::log("Has complied flavors");
		if($hasForced)
			kLog::log("Has forced flavors");
		...
		{
			kLog::log("Lowest flavor selected [$lowestFlavorParamsId]");
