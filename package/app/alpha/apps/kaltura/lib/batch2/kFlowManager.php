...
class kFlowManager
{
	...
	{
		//protected constructor make sure no one can instantiate this class from outside 
	} 
	...
	// make the flow decision - what job should be created next
	public static function updated(BatchJob $dbBatchJob, $entryStatus = null, BatchJob $twinJob = null)
	{
		try
		{
			...
			$kFlowManager = null;
			try{
				$kFlowManager = kConf::get('default_flow_manager_class');
			}
			catch(Exception $e){
				$kFlowManager = 'kFlowManager';
			}
			
			$partner = $dbBatchJob->getPartner();
			if($partner)
			{
				$partnerFlowManager = $partner->getFlowManagerClass();
				if($partnerFlowManager)
					$kFlowManager = $partnerFlowManager;
			}
			
			$flowManager = new $kFlowManager();
			
			switch($jobType)
			{
				case BatchJob::BATCHJOB_TYPE_IMPORT:
					$dbBatchJob = $flowManager->updatedImport($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
			
				case BatchJob::BATCHJOB_TYPE_EXTRACT_MEDIA:
					$dbBatchJob = $flowManager->updatedExtractMedia($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
			
				case BatchJob::BATCHJOB_TYPE_CONVERT:
					$dbBatchJob = $flowManager->updatedConvert($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
			
				case BatchJob::BATCHJOB_TYPE_POSTCONVERT:
					$dbBatchJob = $flowManager->updatedPostConvert($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
			
				case BatchJob::BATCHJOB_TYPE_PULL:
					$dbBatchJob = $flowManager->updatedPull($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_DELETE:
					$dbBatchJob = $flowManager->updatedDelete($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_FLATTEN:
					$dbBatchJob = $flowManager->updatedFlatten($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_BULKUPLOAD:
					$dbBatchJob = $flowManager->updatedBulkUpload($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_DOWNLOAD:
					$dbBatchJob = $flowManager->updatedDownload($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_CONVERT_PROFILE:
					$dbBatchJob = $flowManager->updatedConvertProfile($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_REMOTE_CONVERT:
					$dbBatchJob = $flowManager->updatedRemoteConvert($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_BULKDOWNLOAD:
					$dbBatchJob = $flowManager->updatedBulkDownload($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_PROVISION_PROVIDE:
					$dbBatchJob = $flowManager->updatedProvisionProvide($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_PROVISION_DELETE:
					$dbBatchJob = $flowManager->updatedProvisionDelete($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_CONVERT_COLLECTION:
					$dbBatchJob = $flowManager->updatedConvertCollection($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_STORAGE_EXPORT:
					$dbBatchJob = $flowManager->updatedStorageExport($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
					
				case BatchJob::BATCHJOB_TYPE_STORAGE_DELETE:
					$dbBatchJob = $flowManager->updatedStorageDelete($dbBatchJob, $dbBatchJob->getData(), $entryStatus, $twinJob);
					break;
	...
	protected static function alert(BatchJob $dbBatchJob, Exception $exception)
	{
		...
		kLog::log ( "Alert: Error in job [{$dbBatchJob->getId()}]\n".$exception);
