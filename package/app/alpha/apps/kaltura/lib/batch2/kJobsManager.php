...
class kJobsManager
{
	...
	public static function updateBatchJob(BatchJob $batchJob, $status, BatchJob $twinJob = null)
	{
		...
		return kFlowManager::updated($batchJob, null, $twinJob);
	}
	...
	{		
		...
		kLog::log("Using conversion engine [$currentConversionEngine]");
		...
		if(!$dbConvertCollectionJob)
		{
			...
			if($parentJob)
			{
				...
				kLog::log("Created from parent convert job with entry id [" . $dbConvertCollectionJob->getEntryId() . "]");
			}
			else
			{
				...
				kLog::log("Created from convert collection job with entry id [" . $dbConvertCollectionJob->getEntryId() . "]");
			}
		}
		
		kLog::log("Calling CDLProceessFlavorsForCollection with [" . count($finalFlavorParamsOutputs) . "] flavor params");
	...
	public static function addFlavorConvertJob(FileSyncKey $srcSyncKey, flavorParamsOutput $flavor, $flavorAssetId, $mediaInfoId = null, BatchJob $parentJob = null, $lastEngineType = null, BatchJob $dbConvertFlavorJob = null)
	{
		...
		kLog::log("Conversion engines string: '" . $flavor->getConversionEngines() . "'");
		
		// parse supported engine types
		$conversionEngines = explode(',', $flavor->getConversionEngines());
		kLog::log(count($conversionEngines) . " conversion engines found for the flavor");
		
		$currentConversionEngine = reset($conversionEngines); // gets the first engine type
		if(! is_null($lastEngineType))
		{
			kLog::log("Last used conversion engine is [$lastEngineType]");
			...
			{
				kLog::log("There is no other conversion engine to use");
				return null;
			}
		}
		kLog::log("Using conversion engine [$currentConversionEngine]");
		...
		if(is_null($dbConvertFlavorJob))
		{
			if($parentJob)
			{
				...
				kLog::log("Created from parent convert job with entry id [" . $dbConvertFlavorJob->getEntryId() . "]");
			}
			else
			{
				...
				kLog::log("Created from flavor convert job with entry id [" . $dbConvertFlavorJob->getEntryId() . "]");
			}
		}
	...
	public static function addConvertProfileJob(BatchJob $parentJob = null, $entryId, $partnerId, $flavorAssetId, $inputFileSyncLocalPath)
	{
	...
	{
		kLog::log(__METHOD__ . " entryId[$entryId], partnerId[$partnerId], externalStorage id[" . $externalStorage->getId() . "], fileSync id[" . $fileSync->getId() . "], srcFileSyncLocalPath[$srcFileSyncLocalPath]");
		...
		kLog::log("Creating Storage export job, with source file: " . $netStorageExportData->getSrcFileSyncLocalPath()); 
	...
	public static function addStorageDeleteJob(BatchJob $parentJob, Partner $partner, FileSyncKey $syncKey)
	{
		...
		kLog::log("Creating Net-Storage Delete job, with source file: " . $netStorageDeleteData->getSrcFileSyncLocalPath()); 
...
//			throw new APIException(APIErrors::PARTNER_NOT_SET);
		...
		$batchJob->setWorkGroupId($partner->getWorkGroupId());
