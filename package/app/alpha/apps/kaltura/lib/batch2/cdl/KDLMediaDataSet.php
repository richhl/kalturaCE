...
class KDLMediaDataSet  {
	...
	public function ToTags($tagList)
	{
		...
		foreach($tagList as $tag) {
			switch($tag){
				case "flv":
					if($this->_container->IsFormatOf(array("flash video", "flv", "f4v","flash","flashvideo")))
						$tagsOut[] = $tag;
					break;
				case "web":
				case "mbr":
...
//					$formats = array("flash video", "flv", "f4v","flash","flashvideo");
					if($this->_container->IsFormatOf(array("flash video", "flv", "f4v","flash","flashvideo")))
						...
						$contFormats = array("mpeg-4");
						if($this->_container->IsFormatOf($contFormats)) {
							$vidFormats = array("avc","avc1","h264","h.264");
							$audFormats = array("mpeg audio","mp3","aac");
							if($this->_video || $this->_audio) {
								if((!$this->_video || ($this->_video && $this->_video->IsFormatOf($vidFormats)))
								&& (!$this->_audio || ($this->_audio && $this->_audio->IsFormatOf($audFormats)))){
									$tagsOut[] = $tag;
								}
							}
						}
					...
					break;
				case "slweb":
					if($this->_container->IsFormatOf(array("windows media")))
						$tagsOut[] = $tag;
					break;
				case "itunes":
					$formats = array("mpeg-4","mpeg audio", "aiff", "wave");
					if($this->_container->_id=="qt" 
					|| $this->_container->IsFormatOf($formats))
						$tagsOut[] = $tag;
					break;
				default:
					break;
