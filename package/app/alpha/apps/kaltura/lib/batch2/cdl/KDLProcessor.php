...
include_once("KDLTranscoderCommand.php");
	...
	class KDLProcessor  {
		 ...
		 */
		public static function GenerateAll(KDLMediaDataSet $mediaSet, KDLProfile $profile, array &$targetList, array &$errors, array &$warnings)
		{
		 ...
		 */
		public function InitializeFromString($srcMediaInfo) {
			$mdLoader = new KDLMediaInfoLoader($srcMediaInfo);
			$mdLoader->Load($this->_srcDataSet);
...
kLog::log(__METHOD__."-->\n".$flavor->_transcoders[KDLTranscoders::EE3]."\n<--");
					$ee3 = new SimpleXMLElement($flavor->_transcoders[KDLTranscoders::EE3]);
