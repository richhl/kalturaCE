...
*/
	...
	public static function deleteEntry ( entry $entry , $partner_id = null )
	{
		...
		kLog::log("myEntryUtils::delete Entry [" . $entry->getId() . "] Partner [" . $entry->getPartnerId() . "]");
		...
		kLog::log("media type [$media_type]");
		switch ( $media_type )
		{
...
//		echo $deleted_file_path . "\n";
	...
	public static function createThumbnailFromEntry ( entry $entry , entry $source_entry, $time_offset )
	{
		...
		$source_entry_data_key = $source_entry->getSyncKey(entry::FILE_SYNC_ENTRY_SUB_TYPE_DATA);
		$dataPath = kFileSyncUtils::getReadyLocalFilePathForKey($source_entry_data_key);
	...
	static public function setEntryTypeAndMediaTypeFromFile(entry $entry, $entry_full_path)
	{
		...
		{
			if ($entry->getMediaType() != entry::ENTRY_MEDIA_TYPE_SWF && 'swf' != pathinfo($entry_full_path, PATHINFO_EXTENSION))
			{
				$entry->setMediaType(entry::ENTRY_MEDIA_TYPE_DOCUMENT);
			}
			else
			{
				$entry->setMediaType(entry::ENTRY_MEDIA_TYPE_SWF);
			}
			return;
		}
		...
		if ($entry->getType() == entry::ENTRY_TYPE_AUTOMATIC)
		{
			...
			else
			{
				...
				if ('swf' == pathinfo($entry_full_path, PATHINFO_EXTENSION))
				{
					$entry->setMediaType(entry::ENTRY_MEDIA_TYPE_SWF);
				}
				else
...
//print_r ( $entry );			
	...
	public static function copyEntry(entry $entry, Partner $toPartner = null)
 	{
 		kLog::log("copyEntry - Copying entry [".$entry->getId()."] to partner [".$toPartner->getId()."]");
 		$newEntry = $entry->copy();
		...
		$defaultCategoryFilter->remove(categoryPeer::PARTNER_ID);
 		...
 		kLog::log("copyEntry - New entry [".$newEntry->getId()."] was created");
		...
		if($shouldCopyDataForNonClip || $shouldCopyDataForClip)
		{
			...
			$from = $entry->getSyncKey(entry::FILE_SYNC_ENTRY_SUB_TYPE_DATA); // replaced__getDataPath
	 		...
	 		kLog::log("copyEntriesByType - copying entry data [".$from."] to [".$to."]");
		...
		if(kFileSyncUtils::fileSync_exists($ismFrom))
		{
			...
			kLog::log("copying entry ism [".$ismFrom."] to [".$ismTo."]");
		...
		if(kFileSyncUtils::fileSync_exists($ismcFrom))
		{
			...
			kLog::log("copying entry ism [".$ismcFrom."] to [".$ismcTo."]");
		...
		if ( $considerCopyThumb ) 
		{
			...
			if(!$skipThumb)
			{
				...
				kLog::log("copyEntriesByType - copying entry thumbnail [".$from."] to [".$to."]");
				kFileSyncUtils::softCopy($from, $to);
			}
		}
