		...
		if ( $object_data instanceof BaseObject )
		{
			...
			if ( $object_data instanceof entry )
			{
				if (defined("KALTURA_API_V3"))
				{
					...
					{
						$puser_id = null;
						kLog::log ( __CLASS__.'::'.__METHOD__.' [line: '.__LINE__.'] could not find kuser ['.$object_data->getKuserId().'] from object ['.$object_data->getId().']');
					}
				}
				else
				{
					...
					if(is_null($puser_id))
					{
						kLog::log ( __CLASS__.'::'.__METHOD__.' [line: '.__LINE__.'] could not get puser_id out of api_v3 context puserId from entry:['.$object_data->getPuserId().'] kuser ID:['.$object_data->getKuserId().'] entry:['.$object_data->getId().']');
					}
				}
			}
		}
