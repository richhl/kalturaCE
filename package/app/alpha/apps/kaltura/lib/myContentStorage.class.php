...
*/
	...
	public static function moveFile($from, $to, $override_if_exists = false, $copy = false )
	{
		...
		kLog::log("myContentStorage::moveFile ($copy): $from to $to");
		
		if ( file_exists( $from ))
		{
			kLog::log(__METHOD__." - $from file exists");	
		}
		else
		{
			kLog::log(__METHOD__." - $from file doesnt exist");	
		}
	...
	{
		$fileExts = array( "flv" , "asf", "wmv", "qt" , "mov" , "mpg", "mpeg" , "avi" , "mp3", "wav" , "wma" ,  "mp4", "m4v", "3gp" , "vob", "f4v", "amr", "mkv" );
		return in_array($ext, $fileExts);
	}
