...
//		return array ( $dc_id , $dc );
	...
	{
		kLog::log(__METHOD__." - file_sync [{$file_sync->getId()}]]");
	...
	public static function getRedirectExternalUrl ( FileSync $file_sync , $additional_url = null )
	{
		...
		kLog::log ( __METHOD__ . ": URL to redirect to [$remote_url]" );
	...
	{
		kLog::log(__METHOD__." - fileSync [{$fileSync->getId()}]");
	...
	public static function retrieveFileFromRemoteDataCenter ( FileSync $file_sync )
	{
		...
		{ 
			kLog::log(__METHOD__." - executing " . $cmd_line);
			exec($cmd_line);
		}
		else {
			kLog::log(__METHOD__." - already exists in temp folder [{$local_file_path}]");
		}
	...
	{
		kLog::log(__METHOD__." - file_sync_id [$file_sync_id], file_hash [$file_hash]");
		...
		kLog::log(__METHOD__." - serving file from [".$file_sync->getFullPath()."]");
