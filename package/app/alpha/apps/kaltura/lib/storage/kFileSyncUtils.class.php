...
class kFileSyncUtils
{
	...
	{
		kLog::log(__METHOD__." - key [$key], fetch_from_remote_if_no_local [$fetch_from_remote_if_no_local]");
		list ( $file_sync , $local ) = self::getReadyFileSyncForKey( $key , $fetch_from_remote_if_no_local , false  );
		if ( ! $file_sync ) 
		{
			kLog::log(__METHOD__." - FileSync not found");
			return false;
		}
		else
		{
		...
		kLog::log(__METHOD__." - file_exists? [$file_exists]");
	...
	public static function getContentsByFileSync ( FileSync $file_sync , $local = true , $fetch_from_remote_if_no_local = true , $strict = true )
	{
		if ( $local )
		{
			...
			{
				kLog::log(__METHOD__." - file was found locally at [$real_path]");
				return file_get_contents( $real_path);
			}
			else
			{
				kLog::log(__METHOD__." - file was not found locally");
				if ( $strict )
				{
	...
	{
		kLog::log(__METHOD__." - key [$key], fetch_from_remote_if_no_local [$fetch_from_remote_if_no_local], strict [$strict]");
	...
	{
		kLog::log(__METHOD__." - key [$key], strict [$strict]");
		...
		if ( $res && $strict )
		{
			kLog::log(__METHOD__." - file already exists");
		...
		{
			kLog::log(__METHOD__." - creating directory for file");
		...
		self::createSyncFileForKey( $key , $strict , $res != null );
	}
	...
	public static function moveToFile ( FileSyncKey $source_key , $target_file_path, $delete_source = true , $overwrite = true)
	{
		...
		{
			kLog::log('could not load ready file sync for key '.$source_key);
			return false;
		}
		...
		{
			kLog::log(__METHOD__." - $target_file_path file exists");	
			if(!$overwrite)
			{
				kLog::log("target [$target_file_path] exists, not overwriting");
				return false;
			}
		...
		{
			kLog::log(__METHOD__." - $target_file_path file doesnt exist");	
		}
		...
		if($copyResult)
		{
			...
			kLog::log("successfully copied file to [$target_file_path] and updated fileSync");
			return true;
		}
		else
		{
			kLog::log("copy failed - not changing filesync");
			return false;
		}
	}
	...
	{
		kLog::log(__METHOD__." - move file: [$temp_file_path] to key [$target_key], ");
		...
		if(!$targetFullPath)
		{
			...
			kLog::log(__METHOD__." - Generated new path [$targetFullPath]");
		}
		...
		{
			kLog::log(__METHOD__." - $temp_file_path file exists");	
		}
		else
		{
			kLog::log(__METHOD__." - $temp_file_path file doesnt exist");	
		}
		...
		{
			kLog::log(__METHOD__." - could not move file from [$temp_file_path] to [{$targetFullPath}]");
	...
	{
		kLog::log(__METHOD__." - copy to url: source_key [$source_key], target_key [$target_key]");
		...
		if ( !$srcRes )
		{
			kLog::log(__METHOD__." - file does not exists");
		...
		if ( $destRes && $strict )
		{
			kLog::log(__METHOD__." - url already exists");
			throw new Exception ( "key [" . $target_key . "] already exists");
		}
	...
	{
		kLog::log(__METHOD__." - copy file: [$temp_file_path] to key [$target_key], ");
	...
	public static function getReadyFileSyncForKey ( FileSyncKey $key , $fetch_from_remote_if_no_local = false , $strict = true )
	{
		...
		if ( $file_sync_list == null )
		{
			kLog::log(__METHOD__." - FileSync was not found"); 
			if ( $strict )
				throw new Exception ( "Cannot find ANY FileSync for " . ( $key ) );
			else 
		...
		{
			if ($local)
				kLog::log(__METHOD__." - FileSync was found locally");
			else
				kLog::log(__METHOD__." - FileSync was found but doesn't exists locally");
				
			return array ( $desired_file_sync , $local );
		}		
		
		kLog::log(__METHOD__." - exact FileSync was not found");
	...
	{
		kLog::log(__METHOD__." - key [$key], strict[$strict], already_exists[$already_exists]");
	...
	public static function createPendingExternalSyncFileForKey(FileSyncKey $key, StorageProfile $externalStorage)
	{
		...
		kLog::log(__METHOD__." - key [$key], externalStorage [$externalStorageId]");
	...
	{
		kLog::log(__METHOD__." - target_key [$target_key], source_key [$source_key]");
		// TODO - see that if in strict mode - there are no duplicate keys -> update existing records AND set the other DC's records to PENDING
		$dc = kDataCenterMgr::getCurrentDc();
		$dc_id = $dc["id"];
		...
		if (!$sourceFile)
		{
			kLog::log(__METHOD__." - Warning: no source but NOT strict. target_key [$target_key], source_key [$source_key] ");
			return null;
		}
	...
	{
		kLog::log(__METHOD__." - FileSync id [" . $file_sync->getId() . "]" );
		return self::retrieveObject( $file_sync->getObjectType(), $file_sync->getObjectId() );
	}
	...
	{
		return self::retrieveObject( $sync_key->object_type, $sync_key->object_id );
	}
	...
	public static function retrieveObject( $object_type, $object_id )
	{
		switch ( $object_type )
		{
			...
			case FileSync::FILE_SYNC_OBJECT_TYPE_FLAVOR_ASSET:
				...
				$object = flavorAssetPeer::retrieveById( $object_id );
		...
		if ( $object == null )
		{
			$error = __METHOD__. " Cannot find object type [" . $object_type . "] with object_id [" . $object_id . "]";
			kLog::log($error);
			throw new Exception ( $error );
		}
		return $object;
