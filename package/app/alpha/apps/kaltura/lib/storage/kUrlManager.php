...
class kUrlManager
{
	...
	public static function getUrlManagerByCdn($cdnHost)
	{
		...
		kLog::log("Uses url manager [$class]");
		return new $class();
	}
	...
	public function getFileSyncUrl(FileSync $fileSync)
	{
		...
		if($this->extention && $this->protocol == StorageProfile::PLAY_FORMAT_RTMP && strtolower($this->extention) != 'flv')
			$url = "mp4:$url";
	...
	public function getFlavorAssetUrl(flavorAsset $flavorAsset)
	{
		...
		if($this->extention)
		{
			...
			{
				$url .= '/forceproxy/true';
				if(strtolower($this->extention) != 'flv')
				{
