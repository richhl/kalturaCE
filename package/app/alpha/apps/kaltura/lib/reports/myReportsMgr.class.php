...
//print_r ( $result );		
		...
		$end = microtime(true);
		kLog::log( "getGraph took [" . ( $end - $start ) . "]" );
...
//			return $result[0]; // for total - there is only a single record
		...
		$end = microtime(true);
		kLog::log( "getTotal took [" . ( $end - $start ) . "]" );
		...
		$page_size , $page_index , $order_by , $object_ids = null )
	{
		...
		{
			$end = microtime(true);
			kLog::log( "getTable took [" . ( $end - $start ) . "]" );			
...
//			if ( $file_name ) $file_name .= "_";
		...
		$page_size , $page_index , $order_by , $object_ids = null )
	{
		...
		if ( $total_count )
		{
			kLog::log( "count from cache: [$total_count]" );
			return $total_count;
		}
	...
	//		return StubReports::STUBexecuteQueryByType ( $partner_id , $report_type , $report_flavor , $input_filter  ,		$page_size , $page_index , $order_by );
			...
			$end = microtime(true);
			kLog::log( "Query took [" . ( $end - $start ) . "]" );
			return $res;
		}
		catch ( Exception $ex )
		{
			kLog::log( $ex->getMessage() );
...
kLog::log( "Reports query using database host: [$host] user [" . $db_config["user"] . "]" );
