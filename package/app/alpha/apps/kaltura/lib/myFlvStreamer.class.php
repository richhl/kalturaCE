	...
	private  static function iterateAssets ( $assets , $streamInfo , $addPadding, $echo )
	{
		...
		foreach ( $assets as $asset )
		{
			...
			for ( $i=0 ; $i < $number_of_iterations ; $i++ )
			{
				...
				kLog::log("playing file [$file_name] from [$start_byte] to [$end_byte]");
				...
				kLog::log("file [$file_name]: flv_tag_prev is not null");
...
self::log ( __METHOD__ , "in loop" , $asset_id );
			...
			if(!$flavor_asset_play)
			{
				kLog::log(__METHOD__.' '.__LINE__.' no play flavor asset for entry '.$entry->getId());
			}
			else
			{
			...
			if(!$flavor_asset_edit)
			{
				kLog::log(__METHOD__.' '.__LINE__.' no edit flavor asset for entry '.$entry->getId());
			}
			else
			{
			...
			{
				kLog::log(__METHOD__.' '.__LINE__.' no edit & play flavor assets for entry '.$entry->getId());
				continue;
			}

			$full_file_name = (substr_count($file_name, myContentStorage::getFSContentRootPath()))? $file_name: myContentStorage::getFSContentRootPath().$file_name;
 			$flv_file_name = kFile::fixPath( $full_file_name );
