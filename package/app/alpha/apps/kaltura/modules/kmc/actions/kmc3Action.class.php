...
//				$this->ui_conf_height = 0 ; // $ui_conf->getHeight();
		...
		if(strtotime($partner->getCreatedAt()) >= strtotime('2009-08-27') ||
		   ...
		   in_array( $this->partner_id, kConf::get('kmc_rna_allowed_partners')))
		{
	...
	private function getAllKMCUiconfs()
	{
		$template_partner_id = (isset($this->templatePartnerId))? $this->templatePartnerId: 0;
		$c = new Criteria();
		$c->addAnd(uiConfPeer::TAGS, '%content_'.$this->kmc_content_version.'%', Criteria::LIKE);
		$c->addAnd(uiConfPeer::PARTNER_ID, $template_partner_id );
		$c->addAnd(uiConfPeer::DISPLAY_IN_SEARCH, mySearchUtils::DISPLAY_IN_SEARCH_KALTURA_NETWORK);
		
		$kmc_confs = uiConfPeer::doSelect($c);
		$this->splitKmcUiConfsToCategories($kmc_confs);
	}
	
	private function splitKmcUiConfsToCategories($uiconfs)
	{
		$this->content_drilldown = array();
		$this->content_moderation = array();
		$this->flavorPreview = array();
		$this->preview_players = array();
		$this->preview_playlists  = array();
		
		foreach($uiconfs as $conf)
		{
			if(strpos($conf->getTags(), 'previewplayer'))
				$this->preview_players[] = $conf;
			if(strpos($conf->getTags(), 'previewplaylist'))
				$this->preview_playlists[] = $conf;
			if(strpos($conf->getTags(), 'contentdrill') && !count($this->content_drilldown))
			{
				$this->content_drilldown[] = $conf;
			}
			if(strpos($conf->getTags(), 'contentmoder8') && !count($this->content_moderation))
			{
				$this->content_moderation[] = $conf;
			}
			if(strpos($conf->getTags(), 'flavorpreview') && !count($this->flavorPreview))
			{
				$this->flavorPreview[] = $conf;
			}
		}
	}
