...
//		$c->addAnd ( PartnerPeer::ID , $exclude_list , Criteria::NOT_IN );
	...
	private static function getPartnersStats ( $ids , $from_date, $to_date )
	{
		...
		$resultset  = self::executeQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_MEDIA );
		...
		$resultset  = self::executeQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_KDP , true );
	    ...
	    // bandwidth - for date
		$resultset  = self::executeQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_TRAFFIC );
		...
		$resultset  = self::executeQuery ( $connection , $ids_str , $no_date_filter , PartnerActivity::PARTNER_ACTIVITY_TRAFFIC );
		// videos, images, audios, entries, rcs, widgets
		while ($resultset->next())
	    {
	    	$partner_id = $resultset->getInt('PARTNER_ID');
	...
	private static function activeSite ( $connection , &$stats ,  $ids_str , $to_date , $delta , $title )
	{
		...
		$resultset  = self::executeActiveSiteQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_KDP );
	...
	{
		$dateFilter = self::createDateCriteria ( null, $to_date , $delta );
		$resultset  = self::executeActivePublisherQuery ( $connection , $ids_str , $dateFilter , PartnerActivity::PARTNER_ACTIVITY_MEDIA );
