...
class entry extends Baseentry implements ISyncableFile
{
	...
	public function save($con = null)
	{
		...
		if ( $this->isNew() )
		{
			...
			{
				$this->setAccessControlId($this->getPartner()->getDefaultAccessControlId());
			}
		...
		kEventsManager::handleObjectChanged($this, $this->modifiedColumns);
		$res = parent::save( $con );
		if ($is_new)
		{
...
//			$res = myContentStorage::getGeneralEntityPath("entry/data", $this->getIntId(), $this->getId(), $this->getData(), $version);
	 ...
	 * @return FileSync
	 */
	private function getDownloadFileSyncAndLocal ( $version = NULL , $format = null , $sub_type = null )
	{
		$sync_key = null;
		...
		elseif ( $this->getType() == self::ENTRY_TYPE_DOCUMENT && is_null($sub_type) )
		{
			// if document, and not requested download specifically - the version should be used 
			$sync_key = $this->getSyncKey( self::FILE_SYNC_ENTRY_SUB_TYPE_DATA , $version );
		}
		elseif ( $this->getType() == self::ENTRY_TYPE_DOCUMENT && $sub_type == self::FILE_SYNC_ENTRY_SUB_TYPE_DOWNLOAD )
		{
			// if document, and specifically requested download (swf)
