...
class category extends Basecategory
{
	...
	public function save($con = null)
	{
		...
		if (($this->isNew() || $this->isColumnModified(categoryPeer::PARENT_ID)) && $this->getParentId() !== 0)
		{
	...
	private function renameOnEntries()
	{
		...
		*/ 
		...
		$entryFilter->set("_matchor_categories", $this->old_full_name);
		$entryFilter->attachToCriteria($c);
		$entries = entryPeer::doSelect($c);
		kLog::log("category::save - Updating [".count($entries)."] entries");
	...
	private function moveEntriesToParent()
	{
		...
		if ($parentCat)
		{
			$c = new Criteria();
			$entryFilter = new entryFilter();
			$entryFilter->set("_matchor_categories", $this->full_name);
			$entryFilter->attachToCriteria($c);
			$entries = entryPeer::doSelect($c);
			foreach($entries as $entry)
			{
