...
abstract class BasePartner extends BaseObject  implements Persistent {
	...
	protected $partner_group_type = 1;


	
	protected $partner_parent_id = 0;


	
	protected $kmc_version = '1';
	...
	public function setWorkGroupId($v)
	{
		...
		if ($this->work_group_id !== $v) {
			...
			$this->modifiedColumns[] = PartnerPeer::WORK_GROUP_ID;
		}
	...
	public function setPartnerGroupType($v)
	{
		...
		if ($this->partner_group_type !== $v || $v === 1) {
	...
	public function setPartnerParentId($v)
	{
		...
		if ($this->partner_parent_id !== $v || $v === 0) {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->monitor_usage = $rs->getInt($startcol + 36);

			$this->priority_group_id = $rs->getInt($startcol + 37);

			$this->work_group_id = $rs->getInt($startcol + 38);

			$this->partner_group_type = $rs->getInt($startcol + 39);

			$this->partner_parent_id = $rs->getInt($startcol + 40);

			$this->kmc_version = $rs->getString($startcol + 41);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[38] => $this->getWorkGroupId(),
			$keys[39] => $this->getPartnerGroupType(),
			$keys[40] => $this->getPartnerParentId(),
			$keys[41] => $this->getKmcVersion(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[38], $arr)) $this->setWorkGroupId($arr[$keys[38]]);
		if (array_key_exists($keys[39], $arr)) $this->setPartnerGroupType($arr[$keys[39]]);
		if (array_key_exists($keys[40], $arr)) $this->setPartnerParentId($arr[$keys[40]]);
		if (array_key_exists($keys[41], $arr)) $this->setKmcVersion($arr[$keys[41]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PartnerPeer::WORK_GROUP_ID)) $criteria->add(PartnerPeer::WORK_GROUP_ID, $this->work_group_id);
