...
abstract class Basenotification extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->scheduler_id = $rs->getInt($startcol + 12);

			$this->worker_id = $rs->getInt($startcol + 13);

			$this->batch_index = $rs->getInt($startcol + 14);

			$this->processor_expiration = $rs->getTimestamp($startcol + 15, null);

			$this->execution_attempts = $rs->getInt($startcol + 16);

			$this->lock_version = $rs->getInt($startcol + 17);

			$this->dc = $rs->getString($startcol + 18);
