...
abstract class Baseentry extends BaseObject  implements Persistent {
	...
	protected $lastmoderationFlagCriteria = null;
	...
	public function getAvailableFrom($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->available_from === null || $this->available_from === '') {
			return null;
		} elseif (!is_int($this->available_from)) {
						$ts = strtotime($this->available_from);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [available_from] as date/time value: " . var_export($this->available_from, true));
			}
		} else {
			$ts = $this->available_from;
		}
		if ($format === null) {
			return $ts;
		} elseif (strpos($format, '%') !== false) {
	...
	public function setAvailableFrom($v)
	{
		...
		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse date/time value for [available_from] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->available_from !== $ts) {
			$this->available_from = $ts;
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->source_id = $rs->getString($startcol + 18);
			...
			$this->puser_id = $rs->getString($startcol + 44);

			$this->access_control_id = $rs->getInt($startcol + 45);

			$this->conversion_profile_id = $rs->getInt($startcol + 46);

			$this->categories = $rs->getString($startcol + 47);

			$this->categories_ids = $rs->getString($startcol + 48);

			$this->start_date = $rs->getTimestamp($startcol + 49, null);

			$this->end_date = $rs->getTimestamp($startcol + 50, null);

			$this->search_text_discrete = $rs->getString($startcol + 51);

			$this->flavor_params_ids = $rs->getString($startcol + 52);

			$this->available_from = $rs->getTimestamp($startcol + 53, null);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aaccessControl !== null) {
				if ($this->aaccessControl->isModified()) {
	...
	public function getaccessControl($con = null)
	{
				include_once 'lib/model/om/BaseaccessControlPeer.php';

		if ($this->aaccessControl === null && ($this->access_control_id !== null)) {

			$this->aaccessControl = accessControlPeer::retrieveByPK($this->access_control_id, $con);
	...
	public function getmoderationFlags($criteria = null, $con = null)
	{
		...
		if ($this->collmoderationFlags === null) {
			if ($this->isNew()) {
			   $this->collmoderationFlags = array();
				...
				$criteria->add(moderationFlagPeer::FLAGGED_ENTRY_ID, $this->getId());
	...
	public function countmoderationFlags($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/om/BasemoderationFlagPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
	...
	{
		$this->collmoderationFlags[] = $l;
		$l->setentry($this);
	}


	
	public function getmoderationFlagsJoinkuserRelatedByKuserId($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BasemoderationFlagPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collmoderationFlags === null) {
			if ($this->isNew()) {
				...
				$this->collmoderationFlags = moderationFlagPeer::doSelectJoinkuserRelatedByKuserId($criteria, $con);
			}
		} else {
	...
	public function getmoderationFlagsJoinkuserRelatedByFlaggedKuserId($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BasemoderationFlagPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collmoderationFlags === null) {
			if ($this->isNew()) {
				...
				$this->collmoderationFlags = moderationFlagPeer::doSelectJoinkuserRelatedByFlaggedKuserId($criteria, $con);
			}
		} else {
		...
		if ($this->collflavorParamsOutputs === null) {
			if ($this->isNew()) {
			   $this->collflavorParamsOutputs = array();
				...
				$criteria->add(flavorParamsOutputPeer::ENTRY_ID, $this->getId());
		...
		if ($this->collflavorAssets === null) {
			if ($this->isNew()) {
			   $this->collflavorAssets = array();
				...
				$criteria->add(flavorAssetPeer::ENTRY_ID, $this->getId());
	...
	public function getflavorAssetsJoinflavorParams($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseflavorAssetPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collflavorAssets === null) {
			if ($this->isNew()) {
				...
				$this->collflavorAssets = flavorAssetPeer::doSelectJoinflavorParams($criteria, $con);
			}
		} else {
