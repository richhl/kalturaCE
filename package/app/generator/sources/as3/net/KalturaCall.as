	...
	public class KalturaCall extends EventDispatcher {
		...
		protected function kalturaObject2Arrays( obj : Object , prefix : String = null) : Array {	
			...
			for (var i:int=0; i<objKeys.length; i++)
			{
				...
				{
					var objArr : Array = getQualifiedClassName(obj[objKeys[i].toString()]).split("::");
					...
					keyArray[j] = keyArray[j].concat(tempKeyValArr[0]);
					valArray[j] = valArray[j].concat(tempKeyValArr[1]);
					j = valArray.length;
				}
			}
			...
			var objArr : Array = getQualifiedClassName( obj ).split("::");	
		...
		protected function extractArray( arr : Array , prefix : String ) : Array {
			...
			for( var i:int=0; i<arr.length; i++)
			{
				...
				{							
					var tempArr : Array =  extractArray( arr[i] , newPrefix);
				...
				else if( arr[i] != null ) //must be an object
				{
					...
					//var tempPrefix : String = objArr[objArr.length-1];
					var tempArr : Array = kalturaObject2Arrays( arr[i] , newPrefix ); //  + ":" +tempPrefix
