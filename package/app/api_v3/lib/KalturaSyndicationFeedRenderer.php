...
class KalturaSyndicationFeedRenderer implements IKalturaPlaylistUtils
{
	...
	public function __construct($feedId, $ignoreFlavorFilter = false)
	{
		...
		if ( ! self::$databaseManager )
		{
			...
			self::$databaseManager->setConfig(new Zend_Config_Ini($configPath));
	...
	{
		return $kalturaEntry->dataUrl.'/flavor_param_id/'.$this->flavor->getId().'/a.'.$this->flavor->getFormat().'?novar=0';
	}
	...
	private function renderGoogleVideoFeed()
	{
		...
		foreach($this->entries as $entry)
		{
			...
			$googleFeed[] = $this->generateFullXmlNode('video:family_friendly', $this->syndicationFeed->familyFriendly, 3);
