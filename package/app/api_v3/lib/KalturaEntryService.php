...
class KalturaEntryService extends KalturaBaseService 
{
	protected function getEntry($entryId, $version = -1, $entryType = null)
	{
	    ...
	    $entry = KalturaEntryFactory::getInstanceByType($dbEntry->getType(), $this->getKs()->isAdmin());
	...
	protected function prepareEntriesCriteriaFilter(KalturaBaseEntryFilter $filter = null, $partnerIdForScope = null)
	{
		if (!$filter)
	...
	protected function listEntriesByFilter(KalturaBaseEntryFilter $filter = null, KalturaFilterPager $pager = null, $partnerIdForScope = null)
	{
		...
		$c = $this->prepareEntriesCriteriaFilter($filter, $partnerIdForScope);
		
		//$totalCount = entryPeer::doCount($c);
		$totalCount = entryPeer::doCountWithCache( $c );
	...
	protected function countEntriesByFilter(KalturaBaseEntryFilter $filter = null)
	{
		...
		return entryPeer::doCountWithCache( $c );
	}
	...
	{
		if ($dbEntry->getKuserId() != $this->getKuser()->getId() && !$this->getKs()->isAdmin())
		{
			throw new KalturaAPIException(KalturaErrors::INVALID_KS, "", ks::INVALID_TYPE, ks::getErrorStr(ks::INVALID_TYPE));
		}
	}
	...
	protected function updateThumbnailForEntryFromSourceEntry($entryId, $sourceEntryId, $timeOffset, $entryType = null)
	{
		...
		$mediaEntry = KalturaEntryFactory::getInstanceByType($dbEntry->getType(), $this->getKs()->isAdmin());
