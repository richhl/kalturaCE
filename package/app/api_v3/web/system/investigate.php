...
$c = new Criteria();
//$c->add ( BatchJobPeer::DATA , "%\"entryId\";i:" . $entry_id . ";%" , Criteria::LIKE );
$c->add ( notificationPeer::OBJECT_TYPE, kNotificationJobData::NOTIFICATION_OBJECT_TYPE_ENTRY ); 
...
<form id="investigate_entry" action="./investigate.php">
	<label>Entry Id: <input type="text" name="entry_id" value="<?php echo $entry_id ?>"></label>
...
<table cellspacing=0 class="investigate">
 ...
 <h3>General Info:</h3>
	...
	<?php echo investigateHelper::printEntryHeader() . " " . investigateHelper::printEntry ( $entry ) ?>
</table>

<a name="conv"></a>
 <table cellspacing=0 class="investigate">
   <caption>
	<h3>Conversions Log <span>(Total: <?php echo count ($conversions) ?>) &nbsp; [<a href="#top">top</a>]</span></h3>
   This table provides a log of the conversions applied to this entry media file. Every conversion attempt is logged as a new row in the table.
	...
	<?php echo investigateHelper::printConversionHeader(  $entry->getId() , ($entry->getMediaType() == 1 ) && $entry->getStatus() >= 1 ) ; ?>
	<?php
...
<a name="batch"></a>
 <table cellspacing=0 class="investigate">
  <caption>
  <h3>Batch Jobs <span>(Total: <?php echo count ($batch_jobs); ?>) &nbsp; [<a href="#top">top</a>]</span></h3>
...
<?php echo investigateHelper::printBatchjobHeader() ?>
	<?php
	if (!count($batch_jobs))
	{
...
<?php echo investigateHelper::listFilesInDir( "Thumbnail" , $entry_id, $thumbnail_file_dir . "/" . $entry->getId() . "*" , true , "This table describes all the thumbnails created for this entry." ); ?>
</table>
...
<?php echo investigateHelper::listFilesInDir( "Archive" , $entry_id, $archive_path . "/" . $entry->getId() . "*" , true , "This table describes the original files copied to the archive folder." ); ?>
</table>
...
<a name="notify"></a>
 <table cellspacing=0 class="investigate">
  <caption><h3>Notifications <span>Total: <?php echo count ($notifications) ?> &nbsp; [<a href="#top">top</a>]</span></h3>
This table logs all the various notifications sent regarding this entry.</caption>
<?php echo investigateHelper::printNotificationHeader() ?>
	<?php
