...
// --------------------------------- ConvertJob functions 	--------------------------------- //
	...
	function addConvertProfileJobAction($entryId)
	{
		...
		$batchJob = kJobsManager::addConvertProfileJob(null, $entryId, $entry->getPartnerId(), $flavorAsset->getId(), $inputFileSyncLocalPath);
...
//		$c->add(BatchJobPeer::DELETED_AT, null);
		...
		$count = BatchJobPeer::doCount($c, false, myDbHelper::getConnection(myDbHelper::DB_HELPER_CONN_PROPEL2));
		$list = BatchJobPeer::doSelect($c, myDbHelper::getConnection(myDbHelper::DB_HELPER_CONN_PROPEL2));
